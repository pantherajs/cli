CREATE TABLE IF NOT EXISTS forum_permission (
  ...
  CONSTRAINT forum_permission_foreign_key_forum_id FOREIGN KEY (forum_id) REFERENCES forum(id),
  CONSTRAINT forum_permission_foreign_key_role_id  FOREIGN KEY (role_id)  REFERENCES role(id)
);

CREATE INDEX forum_permission_index_forum_id  ON forum_permission(forum_id);
CREATE INDEX forum_permission_index_role_id   ON forum_permission(role_id);
CREATE INDEX forum_permission_index_composite ON forum_permission(role_id, forum_id, can_view);
