CREATE TABLE IF NOT EXISTS topic (
  ...
  CONSTRAINT topic_primary_key
    PRIMARY KEY (id),
  CONSTRAINT topic_foreign_key_forum_id
    FOREIGN KEY (forum_id)
    REFERENCES forum(id),
  CONSTRAINT topic_foreign_key_author_alias_id
    FOREIGN KEY (author_alias_id)
    REFERENCES alias(id)
);

CREATE INDEX topic_index_category_id
  ON topic(forum_id);
CREATE INDEX topic_index_author_alias_id
  ON topic(author_alias_id);
